package com.playata.application
{
   ...
   public class AppErrorReporter implements IErrorReporter
   {
      ...
      public function reportError(param1:String, param2:TypedObject = null, param3:Boolean = true, param4:Boolean = true, param5:* = null) : void
      {
         ...
         if(Environment.panelManager != null && Environment.panelManager.dialogManager != null && Environment.panelManager.dialogManager.lastDialogClassName != null)
         {
            _loc13_ = Environment.panelManager.dialogManager.lastDialogClassName;
         }
         Logger.debug("[Application] Application Error: " + param1);
         if(_globalErrorHandlers.exists(param1))
         {
            (_loc16_ = _globalErrorHandlers.getData(param1))();
            return;
         }
         ...
         {
            if(User.current && (Environment.info.isNativeMobile || Environment.platform.isSteam))
            {
         ...
         {
            _loc8_ = (_loc12_ = param5 as Error).errorID;
            if(_loc14_ = _loc12_.getStackTrace())
            {
               Logger.error("[Application] Uncaught Error: (" + _loc8_ + ") " + _loc14_);
            }
            if(_loc8_ == 1502 && _loc14_ && (_loc14_.indexOf("at com.greensock.core::Animation$/_updateRoot()") != -1 || _loc14_.indexOf("::SkeletonSprite/enterFrame()") != -1))
            {
               EffectsLayer.instance.enabled = false;
               return;
            }
            if(_loc8_ == 1009 && _loc14_ && _loc14_.indexOf("at com.greensock.core::Animation$/_updateRoot()") != -1)
            {
               return;
            }
            if(_loc8_ == 1009 && _loc14_ && _loc14_.indexOf("at com.greensock::TweenMax$/resumeAll()") != -1)
            {
               return;
            }
         }
         if(param3)
         {
            ...
            {
               _loc15_ = Application.userController.user.isDebug;
            }
            if(!Environment.info.isTestMode && !_loc15_)
            {
         ...
         {
            _loc18_ = Application.userController.appUser;
            _loc9_ = (_loc9_ = (_loc9_ = (_loc9_ = (_loc9_ = (_loc9_ = (_loc9_ = (_loc9_ = "") + "--------------------------------------------\n") + " User Info:\n") + "--------------------------------------------\n") + ("Client Version: " + ApplicationConfig.clientVersionString + "\n")) + ("Server: " + ServerInfo.serverId + "\n")) + ("Local Time: " + TimeUtil.localDateTime.timestamp + " - " + TimeUtil.localDateTime.format("D.M.Y h:m:s") + "\n")) + ("Server Time: " + TimeUtil.serverDateTime.timestamp + " - " + TimeUtil.serverDateTime.format("D.M.Y h:m:s") + "\n");
            if(_loc18_ != null)
            {
               _loc9_ = (_loc9_ += "User Id: " + _loc18_.id + "\n") + ("User Email: " + _loc18_.email + "\n");
               if(_loc18_.character != null)
               {
                  _loc9_ = (_loc9_ += "Character Id: " + _loc18_.character.id + "\n") + ("Character Name: " + _loc18_.character.name + "\n");
               }
            }
            if(ViewManager.instance != null)
            {
               _loc9_ += "Active Panel: " + ViewManager.instance.activePanel + "\n";
               if(_loc13_ != null)
               {
                  _loc9_ += "Last Dialog: " + _loc13_ + "\n";
               }
            }
            ...
            _loc9_ += "Error Code: " + param1 + "\n";
            if(param2 != null)
            {
               _loc9_ = (_loc9_ = (_loc9_ += "Data:\n") + ("Last Action: " + ApplicationRequest.lastRequestAction + "\n")) + (param2.asJsonString + "\n");
            }
            if(param5 != null && param5 is Error)
            {
               _loc17_ = param5.errorID;
               _loc6_ = [2068];
               _loc9_ += "Error: " + param5.getStackTrace();
               if(_loc6_.indexOf(_loc17_) != -1)
               {
                  param4 = false;
               }
            }
