package com.playata.application.billing
{
   ...
   public class AppConsumableBillingProductInfo extends ConsumableBillingProductInfo
   {
      ...
      override public function get description() : String
      {
         ...
         {
            _loc2_ = (_loc8_ = new ItemSet(itemSetIdentifier)).name + " (" + LocText.current.text("general/item_set_parts",_loc8_.itemCount) + ")\n";
            _loc3_ = CItemSetTemplate.fromId(_loc8_.identifier);
            for each(var _loc4_ in _loc3_.bonusIds)
            {
               _loc5_ = _loc3_.getBonus(_loc4_);
               _loc2_ += "- ";
               _loc2_ += LocText.current.text("general/item_set_parts",_loc4_);
               _loc2_ += ": ";
               _loc2_ += LocText.current.text("general/item_set_value",Math.round(_loc5_.value * 100),GameUtil.getStatName(_loc5_.type));
               _loc2_ += "\n";
            }
            _loc1_ += "\n\n" + _loc2_;
         }
