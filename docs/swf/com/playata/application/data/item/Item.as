package com.playata.application.data.item
{
   ...
   public class Item extends DOItem
   {
      ...
      public function Item(param1:Object, param2:String = null, param3:String = null)
      {
         super(param1);
      ...
      public function getMissileDamage(param1:Boolean = false) : int
      {
         var _loc3_:* = null;
         ...
         var _loc6_:Number = CConstant.item_missile_damage_factor;
         var _loc10_:int = Math.round(itemLevel * _loc6_);
         var _loc8_:* = 0;
         if(isOwnItem)
         {
            _loc3_ = User.current.character;
            _loc2_ = BattleSkills.instance.getSkillsFromBattleEffectData(_loc3_.battleEffectsData);
            _loc7_ = BattleEffect.battleEffectToBattleSkills(104);
            _loc9_ = 0;
            for each(var _loc5_ in _loc7_)
            {
               for each(var _loc4_ in _loc2_)
               {
                  if(_loc4_.id == _loc5_)
                  {
                     _loc9_ = Number(1 + _loc4_.value1 / 100);
                     _loc8_ += _loc10_ * _loc9_;
                  }
               }
            }
            if(param1 && _loc3_.seasonProgress && _loc3_.seasonProgress.isActive())
            {
               for each(_loc5_ in _loc7_)
               {
                  if(_loc3_.seasonProgress.effectSkillId == _loc5_)
                  {
                     _loc9_ = Number(_loc3_.seasonProgress.effectSkillValue1);
                     _loc8_ += _loc10_ * _loc9_;
                  }
               }
            }
         }
         _loc8_ = Number(Math.round(_loc8_));
         return _loc10_ + _loc8_;
      }
      
      public function get totalStats() : int
      {
         return statStamina + statStrength + statCriticalRating + statDodgeRating + statWeaponDamage + getMissileDamage();
      }
      ...
      public function getStatText(param1:int) : String
      {
         switch(int(param1) - 1)
         {
            ...
            case 5:
               return GameUtil.getStatString(param1,getMissileDamage());
            default:
               return null;
         }
      }
         ...
         if(param2)
         {
            switch(int(param1) - 1)
            {
               ...
               case 4:
                  ...
                  break;
               case 5:
                  _loc5_ = param2.getMissileDamage();
            }
         }
         if(param3)
         {
            switch(int(param1) - 1)
            {
               ...
               case 5:
                  return LocText.current.formatHugeNumber(getMissileDamage() - _loc5_ + _loc6_,true);
            }
         }
         else
         {
            switch(int(param1) - 1)
            {
               ...
               case 5:
                  return LocText.current.formatHugeNumber(getMissileDamage() - _loc5_ + _loc6_,true) + " " + LocText.current.text("general/stat_missile_damage");
            }
         }
         return null;
      }
      ...
      public function get iconImageUrl() : String
      {
         ...
         if(_loc1_.indexOf("surprise_box") != -1)
         {
            _loc2_ += "sb" + _loc1_.substr(0,_loc1_.lastIndexOf("_"));
         }
         ...
         {
            _loc2_ += baseIdentifier;
            if(_addGenderSuffix)
            {
               _loc2_ += "_" + _ownerGender;
            }
         }
         _loc2_ += "_i";
