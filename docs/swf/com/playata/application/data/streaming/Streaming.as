package com.playata.application.data.streaming
{
   ...
   public class Streaming extends DOStreaming
   {
      ...
      public function hasActorPoseToUnlock(param1:int) : Boolean
      {
         ...
         {
            if(!(_loc4_ = getUnlockedActors().exists(param1) && (getUnlockedActors().getData(param1) as StreamingActor).unlockedAnimations.exists(_loc3_)) && _loc2_.getAnimation(_loc3_).unlockType == 4 && _loc2_.getAnimation(_loc3_).unlockValue <= streamingResource)
            {
               return true;
            }
         }
