package com.playata.application.module
{
   ...
   public class AppMainGameModule extends AppModuleBase
   {
      ...
      public function onInitGame() : void
      {
         ...
         new BattleSkills();
         new Marriages();
         new ItemOfferCategories(Core.config.getTypedObject("payment_item_offer_categories"));
      }
      ...
      override public function onRefreshData(param1:Object) : void
      {
         ...
         {
            if(appData.hasData("news",true))
            {
               _user.character.refreshNews(appData.news);
            }
         }
         if(_user != null && _user.character != null)
         {
         ...
         if(appData.hasData("user_story",true))
         {
            UserStories.refreshUserStory(appData.user_story);
         }
         if(appData.hasData("user_stories",true))
         {
            UserStories.refreshUserStories(appData.user_stories);
         }
         if(appData.hasData("reported_user_stories",true))
         {
            UserStories.refreshReportedUserStories(appData.reported_user_stories);
         }
         ...
         if(appData.hasData("server_system_message_teaser_info",true) && _user.character != null)
         {
            _user.character.streams.serverSystemMessageTeaserIdentifier = appData.server_system_message_teaser_info;
         }
         if(appData.hasData("streams_info",true) && _user.character != null)
         {
            _user.character.streams.update(appData.streams_info,_user.character.id);
         }
         if(appData.hasData("server_system_messages",true) && _user.character != null)
         {
            _user.character.streams.serverSystemMessages.update(appData.server_system_messages);
         }
         if(appData.hasData("private_system_messages",true) && _user.character != null)
         {
            _user.character.streams.privateSystemMessages.update(appData.private_system_messages);
         }
         if(appData.hasData("private_conversations",true) && _user.character != null)
         {
            _user.character.streams.privateConversations.update(appData.private_conversations);
         }
         if(appData.hasData("private_conversation_messages",true) && _user.character != null)
         {
            _user.character.streams.privateConversations.updatePrivateConversationMessages(appData.private_conversation_messages,appData.stream_id);
         }
         if(appData.hasData("private_system_message",true) && _user.character != null)
         {
            _user.character.streams.privateSystemMessages.updateMessage(appData.private_system_message);
         }
         if(appData.hasData("private_conversation",true) && _user.character != null)
         {
            _user.character.streams.privateConversations.updatePrivateConversation(appData.private_conversation);
         }
         if(appData.hasData("private_conversation_message",true) && _user.character != null)
         {
            _user.character.streams.privateConversations.updatePrivateConversationMessage(appData.private_conversation_message,appData.stream_id);
         }
         if(appData.hasData("messages_ignored_character_info",true) && _user.character != null)
         {
            _user.character.streams.updateIgnoredCharacterInfo(appData.messages_ignored_character_info);
         }
         ...
         if(appData.hasData("draw_event",true))
         {
            if(_user.character != null)
            {
               _user.character.refreshDrawEvent(appData.draw_event);
            }
         }
         ...
         if(appData.hasData("user_voucher",true))
         {
            if(_user.character != null)
            {
               _user.character.refreshUserVoucher(appData.user_voucher);
            }
         }
         if(appData.hasData("user_vouchers",true))
         {
            if(_user.character != null)
            {
               _user.character.refreshUserVouchers(appData.user_vouchers);
            }
         }
         ...
         if(appData.hasData("global_tournament_end_timestamp",true))
         {
            GlobalTournament.globalTournamentEndTimestamp = appData.global_tournament_end_timestamp;
         }
         ...
         if(appData.hasData("season_progress",true))
         {
            if(_user.character != null)
            {
               _user.character.refreshSeasonProgress(appData.season_progress);
            }
         }
         if(appData.hasData("season_rewards",true))
         {
            if(_user.character != null)
            {
               _user.character.seasonProgress.refreshRewards(appData.season_rewards);
            }
         }
         if(appData.hasData("season_reward",true))
         {
            if(_user.character != null)
            {
               _user.character.seasonProgress.refreshReward(appData.season_reward);
            }
         }
         ...
         if(appData.hasData("streaming",true))
         {
            if(_user.character != null)
            {
               _user.character.refreshStreaming(appData.streaming);
            }
         }
         if(appData.hasData("streaming_show",true))
         {
            if(_user.character != null && _user.character.streaming != null)
            {
               _user.character.streaming.refreshStreamingShow(appData.streaming_show);
            }
         }
         if(appData.hasData("streaming_shows",true))
         {
            if(_user.character != null && _user.character.streaming != null)
            {
               _user.character.streaming.refreshStreamingShows(appData.streaming_shows);
            }
         }
      }
      ...
      {
         ViewManager.instance.baseUserPanel.openStreamsPanel(true);
