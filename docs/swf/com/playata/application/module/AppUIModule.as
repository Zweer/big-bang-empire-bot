package com.playata.application.module
{
   ...
   public class AppUIModule extends AppModuleBase
   {
      ...
      {
         if(_viewManager.activePanel == "create_character" && platform.showIngameServerSelection && !platform.isNutakuCom)
         {
            (_viewManager.activePanelInstance as PanelCreateCharacter).serverSelection.setToDefaultServer();
         }
      ...
      override public function onInit(param1:ApplicationOptions = null) : void
      {
         ...
         {
            if((_loc4_ = _applicationVO.panels.getChildAt(_loc3_)).name.indexOf("panel") >= 0)
            {
         ...
         if(Environment.layout.allowResize)
         {
            ...
            _applicationVO.blockerLayer.blockIndicator.y += LayoutUtil.deviceAdjustedHeightCorrection + AppLayout.friendbarCorrection;
            _applicationVO.blockerLayer.blockBackground.y += LayoutUtil.deviceAdjustedHeightCorrection + AppLayout.friendbarCorrection;
         }
      ...
      {
         var data:AppRequestResponseData = param1;
         try
         {
            ...
            if(Environment.startupParams && Environment.startupParams.indexOf("resource_request=") != -1)
            {
               ...
               if(resourceRequest.indexOf("_3_") != -1)
               {
                  ...
                  {
                     _viewManager.baseUserPanel.openStreamsPanel(true);
                  };
      ...
      override public function onUserLogout() : void
      {
         ...
         if(_viewManager)
         {
            _viewManager.baseUserPanel.streamsPanel.close();
         }
      }
