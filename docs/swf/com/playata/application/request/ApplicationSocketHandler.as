         ...
         switch(param2)
         {
            ...
            case "syncConvention":
               if(param3.convention_id !== undefined && param3.fans_current !== undefined && User.current && User.current.character)
               {
                  ...
                  {
                     if(_loc4_ = User.current.character.getConvention(_loc6_))
                     {
                        _loc4_.update(new TypedObject({"fans_current":_loc11_}));
                        if(ViewManager.instance.activePanel == "quests")
                        {
                           (_loc9_ = ViewManager.instance.activePanelInstance as PanelQuests).conventionButton.refreshFans();
                           if(_loc17_ = Environment.panelManager.dialogManager.getVisibleDialog(DialogQuestBriefing) as DialogQuestBriefing)
                           {
            ...
            case "syncGuildLog":
               ...
               if(_loc10_.hasData("chatMessage") && User.current && User.current.character && User.current.character.hasGuild)
               {
                  ...
                  (_loc14_ = TypedObject.empty).setData(_loc5_,_loc15_);
            ...
            case "syncStreams":
               SyncStates.pendingSyncGame = true;
               if(_loc10_.hasData("messageData") && User.current && User.current.character)
               {
                  User.current.character.streams.privateConversations.addPrivateConversationMessage(new DOPrivateConversationMessage(param3.messageData));
                  ViewManager.instance.baseUserPanel.streamsPanel.refresh();
               }
         }
            ...
            if(param3 && _loc10_.hasData("action"))
            {
               switch(param3.action)
               {
                  case "guildMemberUpdate":
                     ...
                     if(_loc12_ = User.current.character.guild.getMemberById(_loc8_))
                     {
                        (_loc7_ = TypedObject.empty).setData(param3.field,param3.value);
