   ...
   public class AppUiUtil
   {
      ...
      public static function alignVerticallyCentered(param1:IDisplayObject, param2:Number, ... rest) : void
      {
         ...
         while(_loc8_ < rest.length)
         {
            ...
            {
               if((_loc11_ = rest[_loc8_] as IDisplayObject).x < _loc4_)
               {
                  _loc4_ = Number(_loc11_.x);
               }
               if(_loc11_ is ITextField)
         ...
         var _loc9_:Number = (_loc7_ = AlignUtil.alignRectangle("center",new Rectangle(0,1,_loc10_,1),param1.getBounds(param1))).x - _loc4_;
         _loc8_ = 0;
         while(_loc8_ < rest.length)
         {
            if(rest[_loc8_] is Array)
            {
               ...
               {
                  _loc11_.x += _loc9_ + param2;
               }
            }
            else
            {
               _loc11_.x = (_loc11_ = rest[_loc8_] as IDisplayObject).x + (_loc9_ + param2);
            }
            _loc8_++;
         }
      }
