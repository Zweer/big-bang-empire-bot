package com.playata.application.ui.dialogs
{
   ...
   public class DialogAccountSettings extends UiDialog
   {
      ...
      private var _btnMarketing:UiTextButton = null;
      ...
      public function DialogAccountSettings()
      {
         ...
         _btnRevokeConsent = new UiTextButton(_loc4_.btnRevokeConsent,LocText.current.text("dialog/account_settings/button_revoke_consent"),LocText.current.text("dialog/account_settings/button_revoke_consent_tooltip"),onClickDeleteAccount);
         ...
         _btnMarketing = new UiTextButton(_loc4_.btnOptOutMarketing,LocText.current.text("dialog/account_settings/button_opt_out_marketing"),"",onClickMarketing);
         ...
         _btnMarketing.fitText();
         var _loc6_:Vector.<UiTextButton>;
         (_loc6_ = new Vector.<UiTextButton>(0)).push(_btnChangePassword);
         ...
         if(User.current.hasMarketingEmail)
         {
            _btnMarketing.caption = LocText.current.text("dialog/account_settings/button_opt_out_marketing");
         }
         else
         {
            _btnMarketing.caption = LocText.current.text("dialog/opt_in_marketing/title");
         }
         _loc7_.push(_btnMarketing);
         ...
         _loc4_.dialogBackground.height += _loc7_.length * _loc3_;
      }
      ...
      private function onClickMarketing(param1:InteractionEvent) : void
      {
         var interactionEvent:InteractionEvent = param1;
         if(User.current.hasMarketingEmail)
         {
            var optOutMarketing:Function = function():void
            {
               Environment.application.sendActionRequest("optOutUserMarketing",{},handleRequests);
            };
            Environment.panelManager.showDialog(new UiTwoChoiceDialog(LocText.current.text("dialog/confirm_opt_out_marketing/title"),LocText.current.text("dialog/confirm_opt_out_marketing/text"),LocText.current.text("general/button_yes"),LocText.current.text("general/button_no"),optOutMarketing));
         }
         else
         {
      ...
      private function handleRequests(param1:ActionRequestResponse) : void
      {
         ...
         if("optOutUserMarketing" !== _loc2_)
         {
            throw new Error("Unsupported request action \'" + param1.action + "\'!");
         }
         if(param1.error == "")
         {
            Environment.panelManager.showDialog(new UiInfoDialog(LocText.current.text("dialog/opt_out_marketing/success_title"),LocText.current.text("dialog/opt_out_marketing/success_text"),LocText.current.text("general/button_ok")));
            User.current.hasMarketingEmail = false;
            close();
         }
         else
         {
