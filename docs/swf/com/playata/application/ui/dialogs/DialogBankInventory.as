package com.playata.application.ui.dialogs
{
   ...
   public class DialogBankInventory extends UiDialog
   {
      ...
      public function DialogBankInventory()
      {
         ...
         var _loc1_:* = 7;
         _btnUpgrade.realContent.caption.x -= _loc1_;
         _btnUpgrade.realContent.iconPremiumCurrency.x -= _loc1_;
         _btnUpgrade.realContent.cost.x -= _loc1_;
         ...
         panelManager.dragDropManager.dialogDragLayer.x -= Environment.layout.appWidth / 2;
         panelManager.dragDropManager.dialogDragLayer.y -= Environment.layout.appHeight / 2;
      ...
      private function onClickSellAll(param1:InteractionEvent) : void
      {
         ...
         while(i < 18)
         {
            ...
            if(item)
            {
               ...
               {
                  ++itemsToSell;
                  var sellPrice:int = sellPrice + item.sellPrice;
               }
            }
            i++;
         }
         ...
         if(outfits.length > 0)
         {
            ...
            for each(outfit in outfits)
            {
               if(outfitsText != "")
               {
                  var outfitsText:String = outfitsText + "\n";
               }
               outfitsText += " - " + outfit.name;
            }
            if(itemsToSell == 1)
            {
