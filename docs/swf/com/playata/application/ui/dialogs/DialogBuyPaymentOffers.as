package com.playata.application.ui.dialogs
{
   ...
   public class DialogBuyPaymentOffers extends DialogPaymentBase
   {
      ...
      public function DialogBuyPaymentOffers(param1:String = null)
      {
         ...
         var _loc6_:*;
         if((_loc6_ = Boolean(AppConfig.offerwallEnabled && platform.isOfferwallAllowed)) && Environment.info.isNativeMobile)
         {
         ...
         if(_loc2_)
         {
            ...
            _loc3_ = 100;
            vo.dialogBackground.height -= _loc3_;
            vo.dialogBackground.y += _loc3_;
            vo.btnHelp.y += _loc3_;
            vo.btnClose.y += _loc3_;
            vo.txtDialogTitle.y += _loc3_;
            adjustGiftElements();
         }
         ...
         var _loc4_:int = 50;
         var _loc5_:int = 8;
         vo.option2.x += 1 * _loc5_;
         vo.option3.x += 2 * _loc5_;
         vo.option5.x += 1 * _loc5_;
         vo.option6.x += 2 * _loc5_;
         vo.optionSpecial1.x += 3 * _loc5_;
         vo.optionSpecial2.x += 3 * _loc5_;
         vo.optionSpecial3.x += 10 * _loc5_;
         vo.optionSpecial4.x += 10 * _loc5_;
         vo.option1.x -= _loc4_ / 2;
         vo.option2.x -= _loc4_ / 2;
         vo.option3.x -= _loc4_ / 2;
         vo.option4.x -= _loc4_ / 2;
         vo.option5.x -= _loc4_ / 2;
         vo.option6.x -= _loc4_ / 2;
         vo.optionSpecial1.x -= _loc4_ / 2;
         vo.optionSpecial2.x -= _loc4_ / 2;
         vo.optionSpecial3.x -= _loc4_ / 2;
         vo.optionSpecial4.x -= _loc4_ / 2;
      ...
      public static function hasSpecialOfferPaymentMethod(param1:Vector.<IBillingProduct>, param2:String, param3:int) : Boolean
      {
         ...
         for each(var _loc4_ in param1)
         {
            if(_loc4_.paymentMethodId == param3)
            {
               ...
               {
                  if((_loc5_ = _loc4_.info as PremiumSpecialBillingProductInfo).specialOfferId == param2)
                  {
                     return true;
                  }
               }
      ...
      private function get currentSpecialOffers() : Vector.<IBillingProduct>
      {
         ...
         for each(var _loc2_ in _allProducts)
         {
            if(!(DialogPaymentBase._currentPaymentMethod && _loc2_.paymentMethodId != DialogPaymentBase._currentPaymentMethod.id))
            {
               ...
               {
                  if(!(_loc2_.info is AppPremiumSpecialBillingProductInfo && (_loc2_.info as AppPremiumSpecialBillingProductInfo).isStandaloneOffer))
                  {
                     _loc1_.push(_loc2_);
                  }
               }
