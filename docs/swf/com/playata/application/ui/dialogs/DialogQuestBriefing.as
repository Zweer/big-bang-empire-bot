package com.playata.application.ui.dialogs
{
   ...
   public class DialogQuestBriefing extends UiDialog
   {
      ...
      public function DialogQuestBriefing(param1:Object, param2:Function)
      {
         ...
         {
            _btnPreviousQuest.x -= Math.round((neededSpace - currentSpace) / 2);
            _btnNextQuest.x += Math.round((neededSpace - currentSpace) / 2);
         }
      ...
      private function setCurrentData() : void
      {
         ...
         if(_currentData is Quest)
         {
            ...
            if((_currentData as Quest).isUserStoryQuest)
            {
               ...
               _realContent.txtType.x += _realContent.iconUserStoryTag.width / 2;
               _realContent.dialogBackground.height += 60;
               _realContent.btnStartQuest.y += 60;
               _realContent.btnStartQuestPremium.y += 60;
         ...
         switch(param1.action)
         {
            case "startQuest":
               if(param1.error == "")
               {
                  ...
                  if(User.current != null && User.current.character != null && !User.current.character.hasTutorialFlag("first_mission"))
                  {
