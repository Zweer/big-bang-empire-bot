package com.playata.application.ui.dialogs
{
   ...
   import com.playata.application.ui.elements.user_story.UiUserStoryVoting;
   ...
   public class DialogQuestComplete extends UiDialog
   {
      ...
      private var _outcomeScrollContainer:UiLabelScrollContainer = null;
      
      private var _userStoryVoting:UiUserStoryVoting = null;
      ...
      public function DialogQuestComplete(param1:Quest, param2:Function)
      {
         ...
         super(_loc7_);
         _outcomeScrollContainer = new UiLabelScrollContainer(_loc7_.txtOutcome,Direction.VERTICAL,true,false,true,true,true);
         _queued = false;
         ...
         else
         {
            ...
            _outcomeScrollContainer.text = _quest.lostText;
         ...
         if(_loc4_ > 0)
         {
            ...
            {
               _loc10_ = _loc3_.currentStoryDungeon;
            }
            else if(_loc3_.repeatStoryDungeon && _loc3_.repeatStoryDungeon.storyDungeonIndex == _loc4_)
            {
               _loc10_ = _loc3_.repeatStoryDungeon;
            }
            if(_loc10_ && !_loc10_.allPointsCollected())
            {
               _loc7_.txtStoryDungeon.visible = true;
               _loc7_.iconStoryDungeon.visible = true;
               _loc7_.txtStoryDungeon.text = StoryDungeon.getUnlockPoinName(_loc10_.storyDungeonIndex);
               _loc7_.txtStoryDungeon.autoFontSize = true;
               _loc7_.iconStoryDungeon.setUriSprite(StoryDungeon.getUnlockPointImageUrl(_loc10_.storyDungeonIndex),37,37,false,1,null,true);
               _loc5_ = StoryDungeon.getUnlockPoinTooltip(_loc10_.storyDungeonIndex);
         ...
         var _loc9_:int = param1.reward.fanFotoResource;
         _loc7_.iconFanFotoResource.visible = false;
         if(!_loc7_.txtStoryDungeon.visible && _loc9_ > 0 && _loc3_.fanFoto)
         {
            _loc7_.txtStoryDungeon.visible = true;
            _loc7_.iconFanFotoResource.visible = true;
            _loc7_.txtStoryDungeon.text = GameUtil.getFanFotoResourceString(_loc9_);
            _loc7_.txtStoryDungeon.autoFontSize = true;
         }
         ...
         _btnReportUserStory = new UiButton(_loc7_.btnReportUserStory,LocText.current.text("dialog/user_story_report/button_report"),onClickReportUserStory);
         _btnReportUserStory.visible = false;
         _loc7_.txtUserStoryInfo.htmlText = LocText.current.text("screen/quest/user_story/user_generated_content_info");
         _loc7_.txtUserStoryInfo.autoFontSize = true;
         _loc7_.txtUserStoryVotingCaption.visible = false;
         _loc7_.txtUserStoryVotingCaption.text = LocText.current.text("screen/quest/user_story/voting_caption");
         _loc7_.txtUserStoryVotingCaption.autoFontSize = true;
         _btnUserStoryInfo = new UiButton(_loc7_.txtUserStoryInfo,"",onClickUserStoryInfo);
         _btnUserStoryInfo.interactionOverMode = "none";
         _btnUserStoryInfo.visible = false;
         _userStoryVoting = new UiUserStoryVoting(_loc7_.userStoryVoting,onClickUserStoryVoting);
         _userStoryVoting.visible = false;
         if(_quest.isUserStoryQuest)
         {
            _loc8_ = UserStories.getUserStory(parseInt(_quest.identifier));
            _btnReportUserStory.visible = _loc8_ != null && _loc8_.canBeReported();
            _btnUserStoryInfo.visible = true;
            if(_loc8_.status == 2)
            {
               _userStoryVoting.visible = true;
               _btnClose.visible = false;
               _loc7_.txtUserStoryVotingCaption.visible = true;
            }
            else
            {
               _btnClose.y += 60;
            }
            _loc7_.dialogBackground.height += 60;
            _yOffset -= 30;
         }
      }
      ...
      override public function dispose() : void
      {
         ...
         _userStoryVoting.dispose();
         _userStoryVoting = null;
         _outcomeScrollContainer.dispose();
         _outcomeScrollContainer = null;
      ...
      private function onClickUserStoryVoting(param1:int) : void
      {
         Environment.application.sendActionRequest("claimQuestRewards",{
            "discard_item":false,
            "create_new":true,
            "user_story_stars":param1
         },handleRequests);
      }
      ...
      {
         if(_userStoryVoting.visible)
         {
            Environment.application.sendActionRequest("claimQuestRewards",{
               "discard_item":true,
               "create_new":true,
               "user_story_stars":_userStoryVoting.userStoryStars
            },handleRequests);
         }
         else
         {
      ...
      private function onClickReportUserStory(param1:InteractionEvent) : void
      {
         ...
         if(_quest.isUserStoryQuest)
         {
            _loc2_ = UserStories.getUserStory(parseInt(_quest.identifier));
         }
         if(_loc2_)
         {
            Environment.panelManager.showDialog(new DialogReportUserStory(_loc2_,onUserStoryReported,true));
         }
      }
      
      private function onUserStoryReported() : void
      {
         var _loc1_:SymbolDialogQuestCompleteGeneric = _vo as SymbolDialogQuestCompleteGeneric;
         _loc1_.txtOutcome.text = _quest.winText;
      }
      ...
      private function handleRequests(param1:ActionRequestResponse) : void
      {
         ...
         if(param1.error == "")
         {
            ...
            {
               if((_loc5_ = ViewManager.instance.user.character.dating.getLastRepeatDatingStep()).isStarted || _loc5_.isFinished)
               {
