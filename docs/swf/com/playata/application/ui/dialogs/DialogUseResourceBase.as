package com.playata.application.ui.dialogs
{
   ...
   public class DialogUseResourceBase extends UiDialog
   {
      ...
      public function DialogUseResourceBase(param1:Sprite, param2:int, param3:Boolean, param4:String)
      {
         ...
         {
            switch(int(_featureType) - 1)
            {
               case 0:
                  if(ViewManager.instance.activePanelInstance is PanelQuestProgress)
                  {
                     if(!(_loc6_ = ViewManager.instance.activePanelInstance as PanelQuestProgress).durationEvent)
                     {
                        _btnUseResource.buttonEnabled = false;
                     }
                     break;
                  }
                  if(ViewManager.instance.activePanelInstance is PanelDungeon && Environment.panelManager.dialogManager.hasVisibleDialog(DialogDungeonQuestBriefing))
                  {
            ...
            switch(int(_featureType) - 1)
            {
               case 0:
                  if(ViewManager.instance.activePanelInstance is PanelQuestProgress)
                  {
                     ...
                     if(_loc3_.durationEvent && (_loc3_.durationEvent.isFightQuest || _loc3_.durationEvent.isTimeQuest || _loc3_.durationEvent.isStatQuest || _loc3_.durationEvent.isUserStoryQuest))
                     {
