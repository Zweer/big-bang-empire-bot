package com.playata.application.ui.elements.account_settings
{
   ...
   public class UiOptionsBase
   {
      ...
      public function refresh() : void
      {
         ...
         _vo.getChildByName("iconAccountSwitch").visible = Environment.info.isNativeMobile && !AppEnvironment.appPlatform.isNutakuCom || AppEnvironment.appPlatform.isCharacterSelectionAllowed;
      ...
      protected function onClickLogout(param1:InteractionEvent) : void
      {
         ...
         else if(_loc2_.hasTutorialFlag("tutorial_finished") && _loc2_.streaming)
         {
            if(_loc2_.streaming.hasRoomWithoutShow())
            {
               Environment.panelManager.showDialog(new UiTwoChoiceDialog(LocText.current.text("dialog/logout_no_streaming_show/title"),LocText.current.text("dialog/logout_no_streaming_show/text"),LocText.current.text("dialog/logout_no_streaming/button_go_to_streaming"),LocText.current.text("dialog/logout_no_streaming/button_logout"),showWorkPanel,logout));
            }
            else if(_loc2_.streaming.getNextFinishedStreamingShow() != null && _loc2_.streaming.getNextFinishedStreamingShow().remainingSeconds <= 60)
            {
               Environment.panelManager.showDialog(new UiTwoChoiceDialog(LocText.current.text("dialog/logout_streaming_collectable/title"),LocText.current.text("dialog/logout_streaming_collectable/text"),LocText.current.text("dialog/logout_no_streaming/button_go_to_streaming"),LocText.current.text("dialog/logout_no_streaming/button_logout"),showWorkPanel,logout));
            }
            else
            {
               logout();
