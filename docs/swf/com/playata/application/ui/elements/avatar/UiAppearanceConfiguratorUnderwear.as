package com.playata.application.ui.elements.avatar
{
   ...
   public class UiAppearanceConfiguratorUnderwear extends UiAppearanceConfigurator
   {
      ...
      private function updateGenitalButtons() : void
      {
         ...
         {
            _btnRightBra.tooltip = _btnLeftBra.tooltip = LocText.current.text("screen/create_character/hidden_underwear_not_allowed");
         }
         else
         {
            _btnRightBra.tooltip = _btnLeftBra.tooltip = LocText.current.text("screen/create_character/button_switch_look_tooltip");
         }
         if(!AppEnvironment.appPlatform.isHidePantiesAllowed && _currentSettings.panties_type == 0)
         {
            _btnRightPanties.tooltip = _btnLeftPanties.tooltip = LocText.current.text("screen/create_character/hidden_underwear_not_allowed");
         }
         else
         {
            _btnRightPanties.tooltip = _btnLeftPanties.tooltip = LocText.current.text("screen/create_character/button_switch_look_tooltip");
         }
         if(AppEnvironment.appPlatform.isHidePantiesAllowed)
         {
      ...
      private function onClickChangeGenitalsHairColor(param1:UiColorBox) : void
      {
         ...
         _colorBoxPickerGenitalsHair.content.y = _colorBoxIconGenitalsHair.content.y - _colorBoxPickerGenitalsHair.content.height - 10;
      }
