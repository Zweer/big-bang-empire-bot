         ...
         switch(param1.action)
         {
            case "getMissedDuel":
               if(param1.error == "")
               {
                  ...
                  (_loc4_ = new Duel(param1.appResponse.missed_duel)).refreshBattle(param1.appResponse.missed_duel_battle);
                  if(param1.appResponse.hasData("requested_character"))
                  {
                     _loc4_.opponent = Character.fromData(param1.data);
                  }
                  PanelBattle.missedDuel = _loc4_;
                  ViewManager.instance.showPanel("battle");
                  if(param1.appResponse.decrease_counter === true)
                  {
                     User.current.character.missedDuels -= 1;
                  }
                  close();
               }
               else
            ...
            case "startDuel":
               ...
               else if(param1.error == "errStartDuelAttackNotAllowed")
               {
                  ...
                  if((_loc5_ = BonusInfo.getValue(2)) > 0)
                  {
                     _loc2_ *= _loc5_;
                  }
                  Environment.panelManager.showDialog(new UiInfoDialog(LocText.current.text("dialog/duel_attack_not_allowed/title"),LocText.current.text("dialog/duel_attack_not_allowed/text",LocText.current.text("general/attack",_loc2_)),LocText.current.text("general/button_ok")));
               }
            ...
            case "deleteMissedDuels":
               ...
               {
                  _loc6_ = 0;
                  for each(var _loc3_ in currentDuels)
                  {
                     _duels.splice(_duels.indexOf(_loc3_),1);
                     _loc6_++;
                  }
                  User.current.character.missedDuels -= _loc6_;
                  Environment.application.updateData(param1.data);
