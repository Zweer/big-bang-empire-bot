package com.playata.application.ui.elements.goal
{
   ...
   public class UiGoalProgressSummaryCategory
   {
      ...
      {
         var _loc3_:int = _character.finishedGoalValueCount(_categoryIndex);
         var _loc1_:int = Goals.instance.totalGoalValues(_categoryIndex,_character);
         var _loc2_:Number = Math.round(_loc3_ / _loc1_ * 100);
         _progressBar.value = _loc2_;
         _content.txtCategoryName.text = _categoryName;
         _content.txtProgress.text = LocText.current.text("dialog/goals/summary/progress_text",_loc3_,_loc1_,_loc2_);
      ...
      private function onClickSwitch(param1:InteractionEvent) : void
      {
         ...
         _onClickSwitchCategory(_categoryIndex);
      }
   }
}
