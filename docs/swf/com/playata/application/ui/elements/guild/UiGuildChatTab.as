   ...
   public class UiGuildChatTab
   {
      ...
      public function UiGuildChatTab(param1:SymbolGuildChatTabGeneric, param2:Function)
      {
         ...
         else
         {
            ...
            _content.inputMessage.width -= _content.btnEmoticon.width + 10;
         }
         ...
         if(!AppConfig.emoticonsEnabled)
         {
            ...
            if(_loc6_ > 0)
            {
               _loc4_ += _loc3_.substring(_loc6_);
            }
         ...
         if(Environment.info.isTouchScreen)
         {
            _content.emoticonPicker.x -= 120;
            _content.emoticonPicker.y += 30;
         ...
         _content.txtPrefix.autoAdjustWidthHeight = TextFieldAutoAdjustWidthHeight.HORIZONTAL;
         collapse(true);
         setInputMode(1);
      }
      ...
      public function refreshLocalization() : void
      {
         ...
         if(!AppConfig.emoticonsEnabled)
         {
            ...
            if(_loc4_ > 0)
            {
               _loc2_ += _loc1_.substring(_loc4_);
            }
      ...
      private function processLink(param1:String) : void
      {
         ...
         else if(param1.indexOf("character;") === 0)
         {
            ...
            _loc2_ = param1.substr(10);
            if(isNaN(parseInt(_loc2_)))
            {
               ViewManager.instance.loadCharacterByName(_loc2_,null);
            }
            else
            {
               ViewManager.instance.loadCharacter(parseInt(_loc2_),null);
            }
         }
      ...
      public function setInputMode(param1:int, param2:String = "") : void
      {
         ...
         var _loc3_:int = 1;
         _content.inputMessage.x = _content.txtPrefix.x + _content.txtPrefix.width + _loc3_;
         ...
         if(!StringUtil.isEmpty(currentInput))
         {
            ...
            else
            {
               if(currentInput.toLowerCase().indexOf(CHAT_COMMAND_PRIVATE_MESSAGE_FALLBACK) == 0 || currentInput.toLowerCase().indexOf(CHAT_COMMAND_PRIVATE_MESSAGE) == 0)
               {
                  ...
                  if((_loc6_ = _guild.getMember(_currentPrivateName)) == null)
                  {
                     appendMessage(getMessage("#FF4B4B",LocText.current.text("guild/chat/error_no_such_name",_currentPrivateName)));
                     _content.inputMessage.text = StringUtil.trim(currentInput);
                     return;
               ...
               if(_loc2_.toLowerCase() == CHAT_COMMAND_HELP || _loc2_.toLowerCase() == CHAT_COMMAND_HELP_FALLBACK)
               {
                  ...
                  if(!AppConfig.emoticonsEnabled)
                  {
                     ...
                     if(_loc9_ > 0)
                     {
                        _loc7_ += _loc3_.substring(_loc9_);
                     }
            ...
            _loc11_ = StringUtil.htmlEscape(StringUtil.trim(currentInput));
            _loc4_ = "";
            ...
            if((_loc10_ = getGuildLogMessageFromOwnMessage(_loc11_,_currentInputMode == 3,_loc4_)).isEmote)
            {
               appendMessage(getMessage("#F2964D",_loc10_.message));
            }
            else if(_loc10_.isOfficerMessage)
            {
               appendMessage(getMessage("#7f7ffd",_loc10_.message));
            }
            else if(_loc10_.isPrivateMessage)
            {
               appendMessage(getMessage("#7FCBFD",_loc10_.message));
            }
            else
            {
               appendMessage(getMessage("#FFFFFF",_loc10_.message));
            }
         }
         resetChatInput();
      }
      ...
      private function appendMessage(param1:String) : void
      {
         ...
         if(!_loc2_)
         {
            _loc3_ = Number(_chatOutput.textField.verticalScrollBar.scrollValue);
         }
         _chatOutput.appendText(param1);
         if(_loc2_)
         {
            Runtime.delayFunction(scrollToBottom,0.3);
         }
         else
         {
            _chatOutput.textField.verticalScrollBar.scrollValue = _loc3_;
      ...
      public function addNewChatMessage(param1:GuildLogMessage) : void
      {
         ...
         if(param1.isEmote)
         {
            _loc2_ += getMessage("#F2964D",param1.message);
         }
         else if(param1.isPrivateMessage)
         {
            _loc2_ += getMessage("#7FCBFD",param1.message);
         }
         else if(param1.isOfficerMessage)
         {
            _loc2_ += getMessage("#7f7ffd",param1.message);
         }
         else
         {
            _loc2_ += getMessage("#FFFFFF",param1.message);
         }
         if(_loc2_ != "")
         {
            ...
            case "sendGuildChatMessage":
               ...
               if(response.error == "")
               {
                  ...
                  if(response.appResponse.guild_chat_message != null)
                  {
                     ...
                     var clientMessage:GuildLogMessage = getGuildLogMessageFromOwnMessage(response.request.getString("message"),response.request.getBoolean("officer_message"),response.request.getString("character_to_name"));
                     if(clientMessage.isEmote)
                     {
                        replaceMessage(getMessage("#F2964D",clientMessage.message),getMessage("#F2964D",serverMessage.message));
                     }
                     else if(clientMessage.isOfficerMessage)
                     {
                        replaceMessage(getMessage("#7f7ffd",clientMessage.message),getMessage("#7f7ffd",serverMessage.message));
                     }
                     else if(clientMessage.isPrivateMessage)
                     {
                        replaceMessage(getMessage("#7FCBFD",clientMessage.message),getMessage("#7FCBFD",serverMessage.message));
                     }
                     else
                     {
                        replaceMessage(getMessage("#FFFFFF",clientMessage.message),getMessage("#FFFFFF",serverMessage.message));
                     }
                  }
               }
      ...
      private function getGuildLogMessageFromOwnMessage(param1:String, param2:Boolean, param3:String) : GuildLogMessage
      {
         var _loc6_:GuildMember = User.current.character.guild.getMember(param3,true);
         var _loc4_:int = 0;
         if(_loc6_)
         {
            _loc4_ = _loc6_.id;
         ...
         var _loc5_:Object = {
            "character_from_name":User.current.character.name,
            "character_to_id":_loc4_,
            "is_officer":(!!param2 ? 1 : 0),
            "is_private":(_loc4_ > 0 ? 1 : 0),
            "message":param1,
            "timestamp":TimeUtil.now
         };
         return new GuildLogMessage(_loc5_);
      }
         ...
         for each(var _loc8_ in _guild.logMessages)
         {
            ...
            else if(!_processedLogIds.exists(_loc8_.logId))
            {
               ...
               if(_loc8_.isEvent)
               {
                  ...
                  {
                     _loc2_ += getMessage("#F0CC04",_loc8_.message);
                  }
               }
               else if(_loc8_.isEmote)
               {
                  _loc2_ += getMessage("#F2964D",_loc8_.message);
               }
               else if(_loc8_.isPrivateMessage)
               {
                  _loc2_ += getMessage("#7FCBFD",_loc8_.message);
               }
               else if(_loc8_.isOfficerMessage)
               {
                  _loc2_ += getMessage("#7f7ffd",_loc8_.message);
               }
               else
               {
                  _loc2_ += getMessage("#FFFFFF",_loc8_.message);
               }
               _loc6_++;
