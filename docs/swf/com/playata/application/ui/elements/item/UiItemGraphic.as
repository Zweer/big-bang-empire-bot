package com.playata.application.ui.elements.item
{
   ...
   public class UiItemGraphic extends Sprite implements IDragSource
   {
      ...
      private var _equippedStamina:int = 0;
      
      private var _equippedStrength:int = 0;
      
      private var _equippedCriticalRating:int = 0;
      
      private var _equippedDodgeRating:int = 0;
      
      private var _equippedWeaponDamage:int = 0;
      
      private var _equippedMissileDamage:int = 0;
      ...
      public function handleInteractionUp(param1:InteractionEvent) : void
      {
         ...
         if(Environment.info.isTouchScreen)
         {
            ...
            {
               Runtime.delayFunction(function(param1:UiItemGraphic):void
               {
                  TooltipLayer.instance.tooltip = _tooltip;
                  TooltipLayer.instance.tooltipVisible = true;
                  _tooltip.onVisibleChanged.add(handleItemTooltipVisibleChanged);
                  MessageRouter.dispatch(new Message("ItemMessage.notifyItemInteractionOver",param1,param1));
               },0.05,[this]);
            }
            return;
         }
      ...
      private function handleItemTooltipVisibleChanged(param1:Tooltip) : void
      {
         if(!_tooltip || _tooltip.displayObject != param1.displayObject)
         {
            param1.onVisibleChanged.remove(handleItemTooltipVisibleChanged);
            return;
         }
         if(!param1.visible)
         {
      ...
      public function updateEquippedStats() : void
      {
         var _loc5_:int = 0;
         var _loc6_:int = 0;
         var _loc2_:int = 0;
         ...
         if(_itemSlot.isBagSlotType || _itemSlot.itemSlotType == 99 || _itemSlot.itemSlotType == 0 || _itemSlot.itemSlotType == 100)
         {
            if(_itemSlot.isBagSlotType || _itemSlot.itemSlotType == 0 || _itemSlot.itemSlotType == 100)
            {
               ...
               if(_loc1_ != null)
               {
                  ...
                  _loc8_ = _loc1_.getMissileDamage();
                  if((_loc9_ = ItemSet.getStatValueDiff(UiItemTooltip.compareItem,item)).exists(1))
                  {
                     _loc5_ -= _loc9_.getData(1);
                  }
                  if(_loc9_.exists(2))
                  {
                     _loc6_ -= _loc9_.getData(2);
                  }
                  if(_loc9_.exists(3))
                  {
                     _loc2_ -= _loc9_.getData(3);
                  }
                  if(_loc9_.exists(4))
                  {
                     _loc4_ -= _loc9_.getData(4);
                  }
                  if(_loc9_.exists(5))
                  {
                     _loc7_ -= _loc9_.getData(5);
                  }
                  if(_loc9_.exists(6))
                  {
                     _loc8_ -= _loc9_.getData(6);
                  }
               }
            }
            else
            {
               for each(var _loc3_ in availableItemSlots)
               {
                  if(_loc3_.container.visible && _loc3_.itemSlotType == _item.itemSlotType && _loc3_.item != null)
                  {
                     ...
                     _loc8_ = _loc3_.item.getMissileDamage();
                     if((_loc9_ = ItemSet.getStatValueDiff(UiItemTooltip.compareItem,item)).exists(1))
                     {
                        _loc5_ -= _loc9_.getData(1);
                     }
      ...
      public function refreshItemCompare() : void
      {
         ...
         var _loc1_:int = _item.statCriticalRating + _item.statDodgeRating + _item.statStamina + _item.statStrength + _item.statWeaponDamage + _item.getMissileDamage();
