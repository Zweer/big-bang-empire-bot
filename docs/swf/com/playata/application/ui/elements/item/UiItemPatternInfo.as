package com.playata.application.ui.elements.item
{
   ...
   import com.playata.application.ui.dialogs.DialogItemPatternSource;
   ...
   public class UiItemPatternInfo extends InteractiveDisplayObject
   {
      ...
      public function UiItemPatternInfo(param1:SymbolItemPatternInfoGeneric, param2:DialogSewingMachine)
      {
         ...
         while(_loc3_ <= 12)
         {
            ...
            (_loc5_ = new UiItemSlot(_loc4_,0)).locked = true;
            _itemSlots.push(_loc5_);
            _loc3_++;
         }
         _content.txtNoItems.text = LocText.current.text("dialog/item_pattern/no_Items");
         _content.txtNoItems.visible = false;
      ...
      {
         Environment.panelManager.showDialog(new DialogItemPatternSource(_itemPattern.title,_itemPattern.getSourceInfo(),LocText.current.text("general/button_ok")));
      }
      ...
      private function scrollDown(param1:int, param2:Boolean) : void
      {
         ...
         if(_offset == _itemPattern.getItemIdentifiersFiltered(_loc3_.gender).length - 12)
         {
            return;
         }
         _offset += param1;
         if(_offset > _itemPattern.getItemIdentifiersFiltered(_loc3_.gender).length - 12)
         {
            _offset = _itemPattern.getItemIdentifiersFiltered(_loc3_.gender).length - 12;
            param2 = false;
         }
         if(param2)
         {
      ...
      public function refresh(param1:ItemPattern, param2:Boolean = false) : void
      {
         ...
         _btnSourceInfo.htmlTooltip = _itemPattern.getSourceInfo(10);
         ...
         if(_loc4_)
         {
            ...
            if(_content.rewardTitle.visible)
            {
               ...
               _content.txtReward.x += 35;
               _content.iconItem.x += 35;
               _content.iconQuestEnergy.x += 35;
               _content.iconStatPoints.x += 35;
               _content.itemReward.x += 35;
               _content.tooltipLayer.x += 35;
            }
         ...
         _btnScrollUp.visible = _itemPattern.getItemIdentifiersFiltered(_loc3_.gender).length > 12;
         _btnScrollDown.visible = _itemPattern.getItemIdentifiersFiltered(_loc3_.gender).length > 12;
         _offset = 0;
         refreshList();
         ...
         _content.txtNoItems.visible = _itemPattern.getItemIdentifiersFiltered(_loc3_.gender).length == 0;
      }
      ...
      private function refreshList() : void
      {
         ...
         _btnScrollDown.buttonEnabled = _offset < _itemPattern.getItemIdentifiersFiltered(_loc1_.gender).length - 12;
         var _loc2_:Object = Constants.current.constant("item_templates");
         _loc3_ = 0;
         while(_loc3_ < _itemSlots.length)
         {
            (_loc5_ = _itemSlots[_loc3_]).container.visible = _itemPattern.getItemIdentifiersFiltered(_loc1_.gender).length > _loc3_ + _offset;
            if(_loc5_.container.visible)
            {
               _loc4_ = _itemPattern.getItemIdentifiersFiltered(_loc1_.gender)[_loc3_ + _offset];
               _loc5_.itemPattern = _itemPattern.getItemIdentifiersFiltered(_loc1_.gender)[_loc3_ + _offset] as String;
