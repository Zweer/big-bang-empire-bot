package com.playata.application.ui.elements.leaderboard
{
   ...
   public class UiLeaderboardCharacterContent
   {
      ...
      private function selectCharacterLine(param1:UiLeaderboardCharacterLine) : void
      {
         ...
         _onSelectLine(_selectedCharacterLine != null ? _selectedCharacterLine.character : null);
      }
      ...
      private function retrieveLeaderboard(param1:Object) : void
      {
         if(param1 is RequestData)
         {
            RequestData(param1).setString("server_id",_context.serverId);
         }
         else
         {
            param1.server_id = _context.serverId;
         }
         Environment.application.sendActionRequest("retrieveLeaderboard",param1,handleRequests);
      }
      ...
      private function handleRequests(param1:ActionRequestResponse) : void
      {
         ...
         else if(param1.error == "errRetrieveLeaderboardSync")
         {
            Environment.panelManager.showDialog(new UiInfoDialog(LocText.current.text("error/retrieve_leaderboard_sync_title"),LocText.current.text("error/retrieve_leaderboard_sync_text"),LocText.current.text("general/button_ok")));
         }
         else
         {
