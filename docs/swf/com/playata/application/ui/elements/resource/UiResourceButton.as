package com.playata.application.ui.elements.resource
{
   ...
   public class UiResourceButton extends UiButton
   {
      ...
      private function adjustWidth() : void
      {
         ...
         var _loc5_:ILabel;
         var _loc4_:Number = (_loc5_ = _loc2_.caption).width;
         var _loc1_:Number = _loc5_.textWidth;
         if(_loc1_ > _loc4_)
         {
            _loc3_ = _loc1_ - _loc4_ + 10;
            _loc5_.width += _loc3_;
            _loc5_.x -= Math.round(_loc3_ / 2);
            _loc2_.bg.width += _loc3_;
            _loc2_.bg.x -= Math.round(_loc3_ / 2);
            _loc2_.iconResource.x += Math.round(_loc3_ / 2);
            _loc2_.iconPremiumCurrency.x += Math.round(_loc3_ / 2);
            _loc2_.cost.caption.x += Math.round(_loc3_ / 2);
         }
      }
   }
}
