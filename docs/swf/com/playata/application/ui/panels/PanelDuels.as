package com.playata.application.ui.panels
{
   ...
   public class PanelDuels extends DefaultPanel
   {
      ...
      public function PanelDuels(param1:SymbolPanelDuelsGeneric)
      {
         ...
         {
            _duelStaminaRefillHint.content.x += 55;
         }
      ...
      public static function getDuelResultTooltipBottom(param1:int, param2:int) : String
      {
         ...
         if(_loc4_ == 0)
         {
            _loc3_ += LocText.current.text("screen/duel/tooltip_lose_no_honor");
         }
         else
         {
            _loc3_ += LocText.current.text("screen/duel/tooltip_lose_honor",GameUtil.getHonorString(_loc4_));
         }
         if(User.current.character.getTotalStats(true) < param2)
         {
            _loc3_ += "\n\n";
            _loc3_ += LocText.current.text("screen/duel/tooltip_increased_reward_chance");
         }
         return _loc3_;
      }
      ...
      {
         var _loc4_:String;
         return (_loc4_ = (_loc4_ = (_loc4_ = "") + getDuelResultTooltipTop(param1,param2)) + "\n\n") + getDuelResultTooltipBottom(param2,param3);
      }
      ...
      override public function show(param1:* = null) : void
      {
         ...
         _btnViewPlayer.visible = _btnRefreshDuelEnemies.visible = User.current.character.hasTutorialFlag("first_duel");
         if(!User.current.character.hasTutorialFlag("first_duel"))
         {
            ViewManager.instance.tutorialArrow.showTutorial(LocText.current.text("tutorial/duel_opponent_list_arrow_text"),"right_back","right",380 + LayoutUtil.deviceAdjustedWidthCorrection / 2,302 + LayoutUtil.deviceAdjustedHeightCorrection / 2);
         }
      }
      ...
      {
         _btnAttack.buttonEnabled = _btnViewPlayer.buttonEnabled = false;
         _selectedOpponent = null;
         _enemyLine1.refresh();
         _enemyLine2.refresh();
         _enemyLine3.refresh();
      ...
      private function selectEnemyLine(param1:UiDuelEnemyLine) : void
      {
         ...
         if(_selectedOpponent)
         {
            _btnAttack.buttonEnabled = _btnViewPlayer.buttonEnabled = true;
         }
      }
      ...
      private function handleRequests(param1:ActionRequestResponse) : void
      {
         ...
         switch(response.action)
         {
            ...
            case "startDuel":
               ...
               {
                  var attackLimit:int = CConstant.duel_attack_limit;
                  var bonusValue:Number = BonusInfo.getValue(2);
                  if(bonusValue > 0)
                  {
                     var attackLimit:int = attackLimit * bonusValue;
                  }
                  Environment.panelManager.showDialog(new UiInfoDialog(LocText.current.text("dialog/duel_attack_not_allowed/title"),LocText.current.text("dialog/duel_attack_not_allowed/text",LocText.current.text("general/attack",attackLimit)),LocText.current.text("general/button_ok")));
               }
