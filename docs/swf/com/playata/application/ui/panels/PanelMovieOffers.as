package com.playata.application.ui.panels
{
   ...
   public class PanelMovieOffers extends DefaultPanel
   {
      ...
      public function PanelMovieOffers(param1:SymbolPanelMovieOffersGeneric)
      {
         ...
         param1.offersContent.energy.txtEnergyTimer.visible = false;
         param1.offersContent.energy.txtEnergyLeft.y = -15;
         param1.offersContent.energy.btnBuyEnergy.visible = false;
         _tooltipEnergy = new UiTextTooltip(param1.offersContent.energy,"");
      ...
      override public function refreshPanelLocalization() : void
      {
         ...
         {
            _btnRefreshOffers.premiumCaption = _btnRefreshOffers.nonPremiumCaption = LocText.current.text("screen/movie_offers/button_refresh");
         }
      }
      ...
      override public function refresh() : void
      {
         ...
         {
            _loc3_.noOffersContent.visible = false;
            _loc3_.offersContent.visible = true;
            ...
            _loc3_.offersContent.energy.txtEnergyLeft.text = LocText.current.text("general/energy_amount",User.current.character.maxMovieEnergy,User.current.character.maxMovieEnergy);
            _tooltipEnergy.text = LocText.current.text("screen/movie_progress/movie_energy_tooltip_full");
            if(_loc2_.remainingMoviesToday > 0)
            {
               _tooltipEnergy.text += "\n\n" + LocText.current.text("screen/movie_offers/remaining_movies_today",_loc2_.remainingMoviesToday);
            }
         }
         else
         {
            _loc3_.noOffersContent.visible = true;
            _loc3_.offersContent.visible = false;
            onTimerEvent();
            _timer.start();
            if(_loc2_.remainingMoviesToday > 0)
            {
      ...
      override public function show(param1:* = null) : void
      {
         ...
         if(_loc2_.noOffersContent.discountHint.visible)
         {
            ...
            _loc2_.noOffersContent.discountHint.scaleX = _loc2_.noOffersContent.discountHint.scaleY = 1;
      ...
      {
         var _loc4_:SymbolPanelMovieOffersGeneric;
         (_loc4_ = _vo as SymbolPanelMovieOffersGeneric).background.removeAllChildren();
         var _loc2_:Character = User.current.character;
         if(!_loc2_.hasGuild)
         {
            return;
