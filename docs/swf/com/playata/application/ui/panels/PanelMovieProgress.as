package com.playata.application.ui.panels
{
   ...
   public class PanelMovieProgress extends DefaultPanel
   {
      ...
      override public function refresh() : void
      {
         ...
         {
            _loc5_ += "\n\n" + LocText.current.text("screen/movie_progress/progress_until_title",GameUtil.getMovieProgressString(_loc2_.getRemainingEnergyForNextStar));
         }
         else if(_loc2_.claimedStars == 1)
         {
            _loc5_ += "\n\n" + LocText.current.text("screen/movie_progress/progress_until_cover",GameUtil.getMovieProgressString(_loc2_.getRemainingEnergyForNextStar));
         }
         else if(_loc2_.claimedStars == 2)
         {
            _loc5_ += "\n\n" + LocText.current.text("screen/movie_progress/progress_until_complete",GameUtil.getMovieProgressString(_loc2_.getRemainingEnergyForNextStar));
         }
      ...
      private function refreshEnergyInfo() : void
      {
         ...
         if(_loc1_.remainingMoviesToday > 0)
         {
            _tooltipEnergy.text += "\n\n" + LocText.current.text("screen/movie_progress/remaining_movies_today",_loc1_.remainingMoviesToday);
         }
         else
         {
            _tooltipEnergy.text += "\n\n" + LocText.current.text("screen/movie_progress/no_remaining_movies_today");
         }
      }
      ...
      private function loadBackgrounds() : void
      {
         ...
         {
            if((_loc6_ = _loc5_[_loc3_]).indexOf("PLAYER_F") != -1 && _loc1_.gender == "f" || _loc6_.indexOf("PLAYER_M") != -1 && _loc1_.gender == "m")
            {
      ...
      private function handleMessages(param1:Message) : void
      {
         switch(param1.type)
         {
            case "ViewMessage.notifyBattleFinished":
               ...
               if(!User.current.character.movie.activeMovieQuest)
               {
                  return;
               }
