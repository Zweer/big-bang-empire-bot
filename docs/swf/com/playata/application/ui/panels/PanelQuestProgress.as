package com.playata.application.ui.panels
{
   ...
   public class PanelQuestProgress extends DefaultPanel
   {
      ...
      private var _initialButtonInstantFinishY:Number = 0;
      
      private var _initialButtonCancelY:Number = 0;
      
      private var _initialContentY:Number = 0;
      
      private var _initialBackgroundHeight:Number = 0;
      
      private var _descriptionScrollContainer:UiLabelScrollContainer = null;
      ...
      public function PanelQuestProgress(param1:SymbolPanelQuestProgressGeneric)
      {
         ...
         super(param1);
         _initialButtonInstantFinishY = param1.content.btnInstantFinish.y;
         _initialButtonCancelY = param1.content.btnCancel.y;
         _initialContentY = param1.content.y;
         _initialBackgroundHeight = param1.content.bg.height;
         ...
         _btnDrawEvent = new UiDrawEventButton(param1.btnDrawEvent,onClickDrawEvent);
         _btnSeason = new UiSeasonButton(param1.btnSeason,onClickSeason);
         ...
         _btnReportUserStory = new UiButton(param1.content.btnReportUserStory,LocText.current.text("dialog/user_story_report/button_report"),onClickReportUserStory);
         _btnReportUserStory.visible = false;
         _btnUserStory = new UiButton(param1.btnUserStory,LocText.current.text("screen/character/button_user_story_tooltip"),onClickUserStory);
         _btnUserStory.visible = false;
         _leftSideButtons = new ButtonPositionGroup(80,80,5,Direction.VERTICAL);
         _leftSideButtons.addButton(_btnHerobook);
         _leftSideButtons.addButton(_btnEventQuest);
         _leftSideButtons.addButton(_btnDrawEvent);
         _leftSideButtons.addButton(_btnQuestMultitasking);
         _leftSideButtons.addButton(_btnConventionMultitasking);
         _leftSideButtons.addButton(_btnDungeonMultitasking);
         param1.content.txtUserStoryInfo.htmlText = LocText.current.text("screen/quest/user_story/user_generated_content_info");
         param1.content.txtUserStoryInfo.autoFontSize = true;
         _btnUserStoryInfo = new UiButton(param1.content.txtUserStoryInfo,"",onClickUserStoryInfo);
         ...
         _descriptionScrollContainer = new UiLabelScrollContainer(param1.content.txtDescription,Direction.VERTICAL,true,false,true,true,true);
         _dailyBonus = new UiDailyBonusQuests(param1.dailyBonusQuestEnergy,3);
         ...
         _anchorContainer.add(param1.btnUserStory,AnchorPoint.TOP_RIGHT);
         _anchorContainer.add(param1.btnSeason,AnchorPoint.TOP_RIGHT);
         _anchorContainer.add(param1.dailyBonusQuestEnergy,AnchorPoint.TOP_RIGHT);
      ...
      override public function dispose() : void
      {
         ...
         _tooltipTime = null;
         _descriptionScrollContainer.dispose();
         _descriptionScrollContainer = null;
      ...
      override public function show(param1:* = null) : void
      {
         ...
         _loc2_.btnSlotMachine.y = !!_btnVideoAdvertisment.hasPendingVideo ? _loc2_.btnUseResource.y - 140 : Number(_loc2_.btnUseResource.y - 70);
         _clientAbort = false;
         ...
         _descriptionScrollContainer.text = _durationEvent.progressText;
      ...
      public function performFinish() : void
      {
         ...
         if(_durationEvent.isStarted)
         {
            ...
            _btnUseResource.visible = false;
            _btnSlotMachine.visible = false;
            if(_durationEvent.isTimeQuest || _durationEvent.isUserStoryQuest)
            {
         ...
         else if(_durationEvent.isFinished)
         {
            ...
            else if(_durationEvent.isStatQuest)
            {
               if(_statQuestShown == false && !_shownStatQuests.exists(_durationEvent.id))
               {
                  ...
                  if(User.current.showQuestStatAnimation)
                  {
                     ViewManager.instance.showPanel("stat_quest");
                  }
                  else
                  {
                     PanelStatQuest.lastViewedMode = 1;
      ...
      override public function refresh() : void
      {
         ...
         if(_durationEvent is ConventionShow && !_clientAbort && !_finishFired)
         {
            if(!_loc1_.activeConventionShow)
            {
               ...
               {
                  setStage(User.current.character.activeQuest.stage);
               }
               else
               {
         ...
         _btnSlotMachine.visible = _checkForCompleteFired == false && _durationEvent.isStarted && PanelSlotMachine.slotmachineAvailable() && _loc1_.hasTutorialFlag("tutorial_finished") && _loc1_.level >= CConstant.slotmachine_teaser_min_level;
         _btnSlotMachine.buttonEnabled = _btnSlotMachine.visible && _loc1_.level >= CConstant.slotmachine_min_level;
         _btnSlotMachine.refresh();
         _btnEventQuest.visible = _loc1_.hasEventQuest && _loc1_.eventQuest != null && _loc1_.hasTutorialFlag("tutorial_finished");
         _btnDrawEvent.visible = _loc1_.hasDrawEvent && _loc1_.hasTutorialFlag("tutorial_finished");
         _btnSeason.visible = _loc1_.seasonProgress.exists();
         if(_btnSeason.visible)
         {
            _btnSeason.refresh(_loc1_.seasonProgress);
         }
         ...
         {
            _btnDrawEvent.refresh(_loc1_.drawEvent);
         }
         ...
         _btnReportUserStory.visible = false;
         _btnUserStoryInfo.visible = false;
         _loc2_.content.btnInstantFinish.y = _initialButtonInstantFinishY;
         _loc2_.content.btnCancel.y = _initialButtonCancelY;
         _loc2_.content.y = _initialContentY;
         _loc2_.content.bg.height = _initialBackgroundHeight;
         if(_durationEvent is Quest)
         {
            if((_durationEvent as Quest).isUserStoryQuest)
            {
               _loc3_ = UserStories.getUserStory(parseInt((_durationEvent as Quest).identifier));
               _btnReportUserStory.visible = _loc3_ != null && _loc3_.canBeReported();
               _btnUserStoryInfo.visible = true;
               _loc2_.content.btnInstantFinish.y += 60;
               _loc2_.content.btnCancel.y += 60;
               _loc2_.content.y -= 30;
               _loc2_.content.bg.height += 60;
            }
         }
         _btnUserStory.visible = AppConfig.userStoryQuestsEnabled && _loc1_.level >= CConstant.user_story_unlock_level && _loc1_.userStoryActivated;
         refreshProgressText();
         _dailyBonus.refresh();
      }
      ...
      private function onClickConventionMultitasking(param1:InteractionEvent) : void
      {
         ...
         if(User.current.character.multitaskingBoosterActive)
         {
            ...
            {
               setStage(User.current.character.getConvention(User.current.character.conventionId).stage);
               return;
            }
      ...
      private function setStage(param1:int, param2:Boolean = false) : void
      {
         if(param1 > 0 && param1 <= User.current.character.maxQuestStage)
         {
            if(param1 != User.current.character.currentQuestStage)
            {
            ...
            else if(ViewManager.instance.activePanel != "quests")
            {
               ViewManager.instance.showPanel("quests");
            }
         }
      ...
      private function onClickReportUserStory(param1:InteractionEvent) : void
      {
         ...
         if(_durationEvent is Quest && (_durationEvent as Quest).isUserStoryQuest)
         {
            _loc2_ = UserStories.getUserStory(parseInt((_durationEvent as Quest).identifier));
         }
         if(_loc2_)
         {
            Environment.panelManager.showDialog(new DialogReportUserStory(_loc2_,refresh,true));
         }
      }
      ...
      {
         var response:ActionRequestResponse = param1;
