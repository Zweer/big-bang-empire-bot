package com.playata.framework.display
{
   ...
   public class AnchorContainer
   {
      ...
      {
         var _loc5_:AnchorDisplayObject;
         (_loc5_ = new AnchorDisplayObject()).displayObject = param1;
      ...
      public function remove(param1:IDisplayObject) : void
      {
         ...
         if(_loc2_)
         {
            if((_loc4_ = _anchorDisplayObjects.indexOf(_loc2_)) != -1)
            {
      ...
      public function setSize(param1:Rectangle) : void
      {
         ...
         for each(var _loc4_ in _anchorDisplayObjects)
         {
            ...
            {
               _loc4_.displayObject.y += Math.round(_loc7_ * _loc4_.percentage);
            }
            else if(_loc3_ == AnchorPoint.LEFT_FIXED)
            {
               _loc4_.displayObject.x += Math.round(_loc6_ * _loc4_.percentage);
            }
            else if(_loc3_ == AnchorPoint.RIGHT_FIXED)
            {
               _loc4_.displayObject.x += Math.round(_loc2_ * _loc4_.percentage);
            }
            else if(_loc3_ == AnchorPoint.BOTTOM_FIXED)
            {
               _loc4_.displayObject.y += Math.round(_loc8_ * _loc4_.percentage);
            }
            ...
            {
               _loc4_.displayObject.width += Math.round(_loc2_ * _loc4_.percentage);
            }
            else if(_loc3_ == AnchorPoint.TOP_BOTTOM)
            {
               _loc4_.displayObject.height += Math.round(_loc8_ * _loc4_.percentage);
            }
         }
         _currentSize = param1;
      }
