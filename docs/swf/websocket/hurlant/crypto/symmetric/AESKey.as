package websocket.hurlant.crypto.symmetric
{
   ...
   public class AESKey implements ISymmetricKey
   {
      ...
      private function expandKey() : void
      {
         ...
         var _loc4_:* = 0;
         var _loc6_:uint;
         Nr = (_loc6_ = key.length / 4) + 6;
         _loc4_ = _loc6_;
         while(_loc4_ < 4 * (Nr + 1))
         {
            _loc7_ = uint(key[4 * _loc4_ - 4]);
            _loc3_ = uint(key[4 * _loc4_ - 3]);
            _loc5_ = uint(key[4 * _loc4_ - 2]);
            _loc1_ = uint(key[4 * _loc4_ - 1]);
            if(!(_loc4_ % _loc6_))
            {
               _loc2_ = _loc1_;
               _loc1_ = uint(Sbox[_loc7_]);
               _loc7_ = uint(Sbox[_loc3_] ^ Rcon[_loc4_ / _loc6_]);
               _loc3_ = uint(Sbox[_loc5_]);
               _loc5_ = uint(Sbox[_loc2_]);
            }
            else if(_loc6_ > 6 && _loc4_ % _loc6_ == 4)
            {
               _loc7_ = uint(Sbox[_loc7_]);
               _loc3_ = uint(Sbox[_loc3_]);
               _loc5_ = uint(Sbox[_loc5_]);
               _loc1_ = uint(Sbox[_loc1_]);
            }
