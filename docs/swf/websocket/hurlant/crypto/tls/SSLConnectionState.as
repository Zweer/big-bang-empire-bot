   ...
   public class SSLConnectionState implements IConnectionState
   {
      ...
      public function decrypt(param1:uint, param2:uint, param3:ByteArray) : ByteArray
      {
         ...
         else
         {
            ...
            if(bulkCipher != 0)
            {
               (_loc8_ = new ByteArray()).writeBytes(param3,param3.length - CIPHER_IV.length,CIPHER_IV.length);
               param3.position = 0;
               cipher.decrypt(param3);
               CIPHER_IV = _loc8_;
               ivmode.IV = _loc8_;
         ...
         if(macAlgorithm != 0)
         {
            ...
            (_loc4_ = new ByteArray()).writeBytes(param3,_loc6_,mac.getHashSize());
      ...
      public function encrypt(param1:uint, param2:ByteArray) : ByteArray
      {
         ...
         {
            cipher.encrypt(param2);
            (_loc5_ = new ByteArray()).writeBytes(param2,param2.length - CIPHER_IV.length,CIPHER_IV.length);
            CIPHER_IV = _loc5_;
            ivmode.IV = _loc5_;
         }
         seq_lo++;
