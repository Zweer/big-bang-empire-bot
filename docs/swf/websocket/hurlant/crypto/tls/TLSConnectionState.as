package websocket.hurlant.crypto.tls
{
   ...
   public class TLSConnectionState implements IConnectionState
   {
      ...
      public function decrypt(param1:uint, param2:uint, param3:ByteArray) : ByteArray
      {
         ...
         {
            (_loc7_ = new ByteArray()).writeBytes(param3,param3.length - CIPHER_IV.length,CIPHER_IV.length);
            cipher.decrypt(param3);
            CIPHER_IV = _loc7_;
            ivmode.IV = _loc7_;
         }
         if(macAlgorithm != 0)
         {
            ...
            (_loc4_ = new ByteArray()).writeBytes(param3,_loc6_,hmac.getHashSize());
      ...
      public function encrypt(param1:uint, param2:ByteArray) : ByteArray
      {
         ...
         {
            cipher.encrypt(param2);
            (_loc4_ = new ByteArray()).writeBytes(param2,param2.length - CIPHER_IV.length,CIPHER_IV.length);
            CIPHER_IV = _loc4_;
            ivmode.IV = _loc4_;
         }
         seq_lo++;
