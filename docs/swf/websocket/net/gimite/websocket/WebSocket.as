   ...
   public class WebSocket extends EventDispatcher
   {
      ...
      public function WebSocket(param1:int, param2:String, param3:Array, param4:String, param5:String, param6:int, param7:String, param8:String, param9:IWebSocketLogger)
      {
         ...
         if(param5 != null && param6 != 0)
         {
            ...
            (_loc11_ = new RFC2817Socket()).setProxyInfo(param5,param6);
            _loc11_.addEventListener("socketData",onSocketData);
            rawSocket = socket = _loc11_;
         }
         else
         {
      ...
      public function send(param1:String) : int
      {
         ...
         if(readyState == 1)
         {
            (_loc4_ = new WebSocketFrame()).opcode = 1;
      ...
      public function close(param1:int = 1005, param2:String = "", param3:String = "client") : void
      {
         ...
         try
         {
            ...
            {
               (_loc8_ = new WebSocketFrame()).opcode = 8;
      ...
      private function onSocketConnect(param1:Event) : void
      {
         ...
         {
            _loc3_ += "Sec-WebSocket-Protocol: " + requestedProtocols.join(",") + "\r\n";
         }
         if(headers)
         {
            _loc3_ += headers;
      ...
      private function onSocketData(param1:ProgressEvent) : void
      {
         ...
         for(; _loc5_ < buffer.length; _loc5_++)
         {
            if(headerState < 4)
            {
      ...
      private function validateHandshake(param1:String) : Boolean
      {
         ...
         var _loc8_:* = null;
         var _loc6_:Array;
         if(!(_loc6_ = param1.split(/\r\n/))[0].match(/^HTTP\/1.1 101 /))
         {
         ...
         while(_loc4_ < _loc6_.length)
         {
            ...
            {
               if(!(_loc7_ = _loc6_[_loc4_].match(/^(\S+):(.*)$/)))
               {
         ...
         var _loc5_:String;
         if((_loc5_ = _loc3_["sec-websocket-accept"]) != expectedDigest)
         {
      ...
      {
         var _loc4_:WebSocketEvent;
         (_loc4_ = new WebSocketEvent("close")).wasClean = param1;
         _loc4_.code = param2;
